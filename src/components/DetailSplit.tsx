...
export interface BillData {
  ...
  alerts?: ReactNode;
  // Present only when this invoice arrived by email (invoices.source_email
  // set) — a sub sometimes writes the real instructions in the message
  // itself rather than (or in addition to) the attached document, and
  // until now that text was only ever visible in whoever's inbox
  // forwarded it. Null/undefined for a manual upload — no email pane at
  // all in that case, not even collapsed.
  sourceEmail?: {
    from: string | null;
    to: string | null;
    subject: string | null;
...
export function DetailSplit({
  ...
  return (
    <div ref={outerRef} className="flex min-w-0 flex-1">
      {bill?.sourceEmail && (showEmail ? (
        <div className="flex w-80 flex-none flex-col border-r border-slate-200 bg-white">
          <div className="flex flex-none items-center justify-between gap-2 border-b border-slate-200 px-4 py-2">
            <button
              type="button"
          ...
          <div className="min-h-0 flex-1 overflow-auto p-4">
            <dl className="space-y-1.5 border-b border-slate-100 pb-3 text-xs">
              <div className="flex gap-2">
                <dt className="w-14 flex-none font-semibold text-slate-500">From</dt>
                <dd className="min-w-0 truncate text-slate-700">{bill.sourceEmail.from ?? "—"}</dd>
              </div>
              <div className="flex gap-2">
                <dt className="w-14 flex-none font-semibold text-slate-500">Date</dt>
                <dd className="text-slate-700">
                  <LocalTime iso={bill.sourceEmail.receivedAt} />
                </dd>
              </div>
              <div className="flex gap-2">
                <dt className="w-14 flex-none font-semibold text-slate-500">Subject</dt>
                <dd className="min-w-0 text-slate-700">{bill.sourceEmail.subject ?? "—"}</dd>
              </div>
              <div className="flex gap-2">
                <dt className="w-14 flex-none font-semibold text-slate-500">To</dt>
                <dd className="min-w-0 truncate text-slate-700">{bill.sourceEmail.to ?? "—"}</dd>
              </div>
            </dl>
            <p className="mt-3 whitespace-pre-wrap text-sm text-slate-700">
              {bill.sourceEmail.body ||
                "This email had no message body — the invoice arrived as an attachment only."}
            </p>
          </div>
        </div>
      ) : (
        <div className="flex w-10 flex-none flex-col items-center gap-3 border-r border-slate-200 bg-slate-100 py-3">
          <button
            ...
            title="Show the email this invoice arrived with"
